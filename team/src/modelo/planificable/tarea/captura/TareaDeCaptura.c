...
TareaPlanificable * generarTareaDeCaptura(Entrenador * entrenador, char * pokemon, Coordinate posicionPokemon) {
    ...
    if (posicionOrigen.valida){
        ...
        Instruccion * instruccionFinal = crearInstruccion(
                ...
                (void (*)(void *, ...)) iniciarCaptura,
                string_from_format("Intento de captura de %s en (%d,%d)", pokemon, posicionPokemon.pos_x, posicionPokemon.pos_y),
                argumentosCaptura
                );
...
void iniciarCaptura(Entrenador * entrenador, char * especie, Coordinate * posicionPokemon){
    ...
    PokemonAtrapable * pokemonAtrapable = servicioDeCapturaProcesoTeam->obtenerPokemonAtrapable(servicioDeCapturaProcesoTeam, especie, convertirACoordenada(posicionPokemon));
    ...
    RespuestaBroker respuestaBroker = clienteBrokerV2ProcesoTeam->enviarCatch(
            clienteBrokerV2ProcesoTeam,
            especie,
            posicionPokemon->pos_x,
            posicionPokemon->pos_y
            );
