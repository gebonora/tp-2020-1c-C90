...
static void notificarEjecucion(TareaPlanificable * this, int numeroInstruccion) {
	if (this->estado == EJECUTANDO && this->contadorDeInstrucciones == numeroInstruccion && numeroInstruccion < this->totalInstrucciones - 1) {
...
static int cantidadInstruccionesEjecutadas(TareaPlanificable* this) {
	return this->totalInstrucciones - this->cantidadInstruccionesRestantes(this);
}
...
static TareaPlanificable *new(Instrucciones instrucciones) {
	...
	tarea->cantidadInstruccionesEjecutadas = &cantidadInstruccionesEjecutadas;
